...
(function(global, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? factory(exports) : typeof define === "function" && define.amd ? define(["exports"], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, factory(global["businessCard-drop"] = {}));
  ...
  const apiurl = "https://mcexpochatapis.marketcentral.in";
  ...
  const BusinessCard = ({
    ...
    const [isCameraAvailable, setIsCameraAvailable] = reactExports.useState(false);
    const [cameraPermissionDenied, setCameraPermissionDenied] = reactExports.useState(false);
    ...
    const [isCameraCaptured, setIsCameraCaptured] = reactExports.useState(false);
    reactExports.useEffect(() => {
      if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
        navigator.mediaDevices.enumerateDevices().then((devices) => {
          const videoInputExists = devices.some((device) => device.kind === "videoinput");
          setIsCameraAvailable(videoInputExists);
        }).catch((err) => {
    ...
    reactExports.useEffect(() => {
      getBusiness();
      console.log("Business categories", businessId);
      console.log("Customer ID", customerId);
    }, []);
    const getcardDetails = async () => {
      if (!localStorage.getItem("customerbcardData")) {
        try {
          const res = await fetch(
            `${apiurl}/api/ExpoChat/GetCustomerBC/${customerId}`
            //http://localhost:5098/api/ExpoChat/GetCustomerBC/Cust123
          );
          ...
          if (data.sucess) {
            ...
            if (data.imagePath != null) {
              setSelectedImage(`${apiurl}/${(data == null ? void 0 : data.imagePath) || null}`);
            }
            if (data.categories.length > 0) {
              console.log(data.categories);
              setSelectedCategories(data.categories);
            }
            localStorage.setItem("customerbcardData", JSON.stringify(data));
          } else {
            console.log("no data is present for this customer");
            return;
          }
          return data;
        } catch (err) {
        ...
        const localdata = JSON.parse(localStorage.getItem("customerbcardData"));
        setRetrivedData(localdata);
        setFormData((prev) => ({
          ...prev,
        ...
        if (localdata.imagePath != null) {
          setSelectedImage(`${apiurl}/${(localdata == null ? void 0 : localdata.imagePath) || null}`);
        }
        if (localdata.categories.length > 0) {
          setSelectedCategories(localdata.categories);
        }
    ...
    function getBusiness() {
      if (!localStorage.getItem("business")) {
        fetch("https://www.marketcentral.in/rest/virtualExpo/general/getBusinesses/6").then((response) => response.json()).then((data) => {
          ...
          localStorage.setItem("business", JSON.stringify(categories));
        }).catch((error) => {
          console.error("Error fetching data:", error);
          setLoadingCategories(false);
        });
      } else {
        const localCategories = JSON.parse(localStorage.getItem("business"));
        setCategoryItems(localCategories);
        setLoadingCategories(false);
      }
    }
    reactExports.useEffect(() => {
      if (showcategory && categoryItems.length > 0 && !defaultCategoriesSelected) {
        const randomCategories = [];
        while (randomCategories.length < 3 && randomCategories.length < categoryItems.length) {
          const randomIndex = Math.floor(Math.random() * categoryItems.length);
          const randomCategory = categoryItems[randomIndex].CATEGORY_ID;
          if (!randomCategories.includes(randomCategory)) {
            randomCategories.push(randomCategory);
          }
        }
        setSelectedCategories(randomCategories);
        setDefaultCategoriesSelected(true);
      }
    }, []);
    ...
    const handleFileChange = (e) => {
      ...
      if (file) {
        if (file.size > MAX_FILE_SIZE) {
          setErrors({
            ...errors,
            image: "Image size should be below 2MB."
          });
          return;
        }
    ...
    const handleCategoryChange = (category) => {
      ...
      if (selectedCategories == null ? void 0 : selectedCategories.includes(category)) {
    ...
    const postbusinesscarddata = async (data) => {
      var _a;
      let formData2 = new FormData();
      formData2.append("CustomerId", customerId);
      formData2.append("Name", data.name || "");
      formData2.append("Email", data.email || "");
      formData2.append("PhoneNumber", data.phoneNumber || "");
      formData2.append("Description", data.description || "");
      formData2.append("ExhibitionId", 6);
      formData2.append("VendorId", data.vendorId || null);
      if ((selectedCategories == null ? void 0 : selectedCategories.length) > 0) {
        selectedCategories.forEach((catId) => {
          formData2.append("Categories", catId);
        });
      }
      if (selectedImage) {
        let file;
        if (((_a = fileInputRef.current) == null ? void 0 : _a.files.length) > 0) {
          file = fileInputRef.current.files[0];
        } else {
          const blob = await fetch(selectedImage).then((res2) => res2.blob());
          file = new File([blob], "captured-image.png", {
            type: "image/png"
          });
        }
        formData2.append("ImageFile", file);
      }
      const res = await fetch(`${apiurl}/api/ExpoChat/PostBusinessCard`, {
        method: "POST",
        body: formData2
      });
      const datas = await res.json();
      if (datas.message) {
        localStorage.removeItem("customerbcardData");
        return data;
      }
    };
    const handleSave = async () => {
      ...
      if (inputType === "business-card" && !selectedImage) {
        newErrors.image = "Please provide a Business Card image via camera or file upload.";
        valid = false;
      }
      if (showcategory && selectedCategories.length === 0) {
      ...
      const postabledata = {
        ...
        vendorId: businessId || ""
        // Assuming vendorId is stored in the response
      };
      await postbusinesscarddata(postabledata);
    ...
    return /* @__PURE__ */ jsx("div", {
      ...
      children: /* @__PURE__ */ jsxs("div", {
        ...
        children: [/* @__PURE__ */ jsx("span", {
          ...
          className: `upload-sectionBc ${!isCameraAvailable || cameraPermissionDenied ? "no-camera" : ""}`,
          ...
          }), isCameraAvailable && !cameraPermissionDenied && /* @__PURE__ */ jsxs("button", {
            type: "button",
        ...
        }), inputType === "business-card" && selectedImage != null && /* @__PURE__ */ jsxs("div", {
  ...
  let customerIdState = "";
  let businessIdState = "";
  let categoryVisibleState = false;
  let root = null;
  function setBusinessId(newId) {
    businessIdState = newId;
    console.log("Business ID Updated:", businessIdState);
  }
  function getBusinessId() {
    return businessIdState;
  }
  function closeBusinessCard(targetId) {
    const targetElement = document.getElementById(targetId);
    if (targetElement && root) {
      console.log("Closing BusinessCard...");
      root.unmount();
      root = null;
    }
  }
  function openUploadcard(targetId, categoryVisible = false, businessId = "", customerId = "") {
    console.log("Opening BusinessCard with:", targetId, categoryVisible, businessId, customerId);
    closeBusinessCard(targetId);
    customerIdState = customerId;
    businessIdState = businessId;
    categoryVisibleState = categoryVisible;
    if (!root) {
      renderBusinessCard(targetId, categoryVisibleState);
    }
  }
  function renderBusinessCard(targetId, showcategory = false) {
    const targetElement = document.getElementById(targetId);
    if (!targetElement) {
    ...
    if (root) {
      console.log("BusinessCard already rendered.");
      return;
    }
    console.log("Rendering BusinessCard...");
    root = ReactDOM.createRoot(targetElement);
    root.render(/* @__PURE__ */ jsx("div", {
      children: /* @__PURE__ */ jsx(BusinessCard, {
        showcategory,
        businessId: businessIdState,
        setBusinessId,
        onClose: () => closeBusinessCard(targetId),
        customerId: customerIdState
      })
    }));
  }
  window.MyBusinessCardPlugin = {
    renderBusinessCard,
    closeBusinessCard,
    // ✅ Allows external closing
    setBusinessId,
    // ✅ Allows external updates
    getBusinessId,
    // ✅ Allows external retrieval
    openUploadcard
    // ✅ New function to open BusinessCard externally
  };
  exports2.renderBusinessCard = renderBusinessCard;
